...
import java.util.stream.Stream;
 ...
 public class StreamAPITest {
	 public static void main(String[] args) {
		 ...
		 // Double sorting
		 employeeList.stream().sorted(Comparator.comparing(EmployeeObj::getAge).reversed()
				 .thenComparing(EmployeeObj::getDepartment)).collect(Collectors.toList()).forEach(System.out::println);
		...
		// System.out.println(nameOfDepartment);
		 ...
		 Map<String, Double> avgAge = employeeList.stream()
				 .collect(Collectors.groupingBy(obj -> obj.gender, Collectors.averagingDouble(EmployeeObj::getAge)));
		// System.out.println(avgAge);

		 //Get the Names of employees who joined after 2015.
		 String nameOfEmpAfter2015 = employeeList.stream().filter(obj -> obj.yearOfJoining > 2015)
				 .map(EmployeeObj::getName).collect(Collectors.joining(",","[","]"));
		 //System.out.println(nameOfEmpAfter2015);

		 //Count the number of employees in each department.
		 Map<String, Long> noOfEmpInDept = employeeList.stream()
				 .collect(Collectors.groupingBy(EmployeeObj::getDepartment, Collectors.counting()));
		 ...
		 Map<String, Double> avgSalDept = employeeList.stream()
				 .collect(Collectors.groupingBy(EmployeeObj::getDepartment,Collectors.averagingDouble(EmployeeObj::getSalary)));
